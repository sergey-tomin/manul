...
def start():
    ...
    pointing3311.delay = s3.value()*1000
...
reopen_btn = QtGui.QPushButton('Stop Statistics')

#s3 = pg.SpinBox(value=100, dec=True, step=10, minStep=1e-6, bounds=[10, 10000], suffix='sec', siPrefix=True)
s3 = pg.SpinBox(value=0.1, step=0.1, suffix='sec',bounds=[0.1, 10], siPrefix=True)
...
w1.addWidget(reopen_btn, row=4, col=0)
d3.addWidget(w1)
