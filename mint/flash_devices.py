...
class MPS(Device):
    ...
    def beam_off(self):
        self.mi.set_value(self.server + ".DIAG/BEAMLINES/FLASH/BLOCK_LASER.FLASH2", 1)

    def beam_on(self):
        self.mi.set_value(self.server + ".DIAG/BEAMLINES/FLASH/BLOCK_LASER.FLASH2", 0)
    ...
    def is_beam_on(self):
        val = not(self.mi.get_value(self.server + ".DIAG/BEAMLINES/FLASH/BLOCK_LASER.FLASH2"))
        return val
...
class BPM(Device):
    ...
    def get_charge(self):
        x = 1 #self.mi.get_value(self.server + ".DIAG/CHARGE.ML/" + self.eid + "/CHARGE." + self.subtrain)
        return x
...
class MIOrbit(Device, Thread):
    ...
    def read_orbit(self, reliable_reading, suffix=""):
        ...
        charge = np.ones(len(x))*100 #np.array(charge)[indx]
...
class MIStandardFeedback(Device):
    ...
    def is_running(self):
        status = self.mi.get_value(self.server + ".FEEDBACK/ORBIT.FLASH2/ORBITFEEDBACK/ACTIVATE_FB")
        return status
